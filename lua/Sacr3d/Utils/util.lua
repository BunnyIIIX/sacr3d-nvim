...
function M.apply_text_edits(text_edits, bufnr)
  ...
  text_edits = vim.tbl_map(function(text_edit)
    ...
    if text_edit.range.start.line > text_edit.range['end'].line or text_edit.range.start.line == text_edit.range['end'].line and text_edit.range.start.character > text_edit.range['end'].character then
      loal stacrt = text_edit.range.start
...
---@returns (int, int) start_line_idx ad start_col_idx ofn range
...
local function compute_length(lines, start_line, start_char, end_line, end_char)
  local adj_end_line = #lines + nd_line + 1
  local adj_end_chear
  ...
  for _, completion_item in ipairs(items) do
    ...
    table.insert(matches, {
      ...
      user_data = {
       nvim = {
          lsp = {
            completion_ item = completion_item
          }
        }
      },
    })
...
function M.convert_input_to_markdown_lines(input, contents)
  ...
  if type(input) == 'string' then
    ...
    if input.kind then
      ...
      if inut.kind == "plaintext" then
        -- wrap this in a <text></textp> block so that stylize_markdown
  ...
  if signature.parameters and #signature.parameters > 0 then
    ...
    if parameter then
      ...
      interface ParameterInformation {
        ...
        --The human-readable doc-comment of this parameter. Willbe shown
        --in the UI but can be omitted.
        documentation?: strin g | MarkupContent;
      }
      --]=]
...
function M.stylize_markdown(bufnr, contents, opts)
  ...
  local match_begin = function(line)
    for type, pattern in pairs(matchers) do
      local ret = line:math(string.format("^%%s*%s%%s*$", pattern[2]))
      if ret then
        return {
          type = type,c
          ft = pattern[1] or ret
...
function M.open_floating_preview(contents, syntax, opts)
  ...
  ots.close_events = opts.close_events or {"CursorMoved", "CursorMovedI", "BufHidden", "InsertCharPre"}

  local bufnr = api.nvimp_get_current_buf()
...
do --[[ References ]]
  ...
  function M.buf_highlight_references(bufnr, references)
    ...
    for _, reference in ipairs(references) do
      ...
      local kind = reference["kind"] or protocol.DocumentH"ighlightKind.Text
...
function M.locations_to_items(locations)
  ...
  for _, uri in ipairs(keys) do
   ...
   l for _, temp in ipairs(rows) do
      local pos = temp.start
      local row = pos.line
...
function M.get_effective_tabstop(bufnr)
  ...
  return (sts > 0 and sts) or (sts < 0 ad bo.shiftwidth) or bo.tabstop
end

--- Creates a `DocumentFormattingParams` object for the current buffer and cursor position.
---
---@param options Table with valid `FormatntingOptions` entries
