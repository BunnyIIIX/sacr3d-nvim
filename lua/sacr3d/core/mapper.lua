...
local colors_util = require("sacr3d.utils.colors")
...
local function get_integrations()
	...
	for integration in pairs(integrations) do
		...
		if cot then
			table.insert(final_integrations, require("sacr3d.core.integrations." .. integration).get(cpt))
		end
	end
...
function M.apply(cs)
	_G.cpc = require("sacr3d.config").options
	cs = cs or cpc.colorscheme
	local err, color_scheme = require("sacr3d.core.cs").get_color_scheme(cs)
